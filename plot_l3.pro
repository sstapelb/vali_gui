...
pro plot_taylor_diagram, year,month,day,file1=file1,file2=file2,varname=varname,sat=sat,algo=algo,save_as=save_as,win_nr=win_nr,$
	...
	if keyword_set(save_as) then begin
		if strcompress(save_as,/rem) eq '1' then begin
			save_as = !SAVE_DIR + datum+'_'+strupcase(varname)+'_taylor_diagram_'+sat_name(algo,sat)+'_vs_'+sat_name(reference,sat) + '.eps'
		endif
	endif
	...
	if keyword_set(time_series) then begin
		...
		bild_cci = fltarr(ncov) -999.
		for ii = 0, ncov-1 do begin
			d = get_available_time_series( algo, varname, sat, coverage = coverage[ii], reference = reference, period = period, sav_file = sav_file, found = found)
			if not found then begin
				ok = dialog_message("Sav File not found! "+sav_file)
				return
			endif
			corr_arr[ii] = d.correlation
			bias_arr[ii] = mean(d.stats[6,*])
			stdd_arr_cci[ii] = d.cci_stdd
			stdd_arr_gac[ii] = d.gac_stdd
			bild_cci[ii] = max(d.cci_all)
		endfor
	endif else begin
		...
		bring_to_same_grid_and_unit,varname,bild_cci,bild_gac,fillv_b_cci,fillv_b_gac,file1=file1,file2=file_ref, $
		...
		if idx_cnt gt 0 then begin
			corr_arr[0]     = correlate(bild_cci[idx],bild_gac[idx])
			bias_arr[0]     = gbias(bild_cci[idx],bild_gac[idx],lat[idx])
			stdd_arr_cci[0] = stddev(bild_cci[idx])
			stdd_arr_gac[0] = stddev(bild_gac[idx])
		endif
		for i = 1, 6 do begin
			case i of
			1    : limit = [-90.0,-180,-65.5,180]
			2    : limit = [-65.5,-180,-23.5,180]
			3    : limit = [-23.5,-180, 23.5,180]
			4    : limit = [ 23.5,-180, 65.5,180]
			5    : limit = [ 65.5,-180, 90.0,180]
...
; 	C_SYMBOL = (["Red","Green","Blue","Goldenrod","Magenta","PUR7","Dark Red","YGB4","Spring Green"])
	...
	bias_perc = bias_arr/max(bild_cci)*100.
	symsize = fltarr(9)
	...
	print,'                       ',strjoin(labels,'            ')
	print,'Norm. STDDEV: ',stddev
	...
	start_save,save_as
		...
		psyms = fltarr(9)
		for i = 0,8 do psyms[i] = cgsymcat(16)
		labels=['G: Global','A: Antarctica','B: MidLat-S','T: Tropics','N: MidLAt-N','C: Arctic',textoidl('P: \pm60\circ Latitude'),'L: Land','S: Sea']
		legend,labels,thick=replicate(2,9),psym=psyms,numsym=1,color=cgcolor(c_symbol), $
		charsize=(keyword_set(save_as) ? 2.5:1.5),pos=[0.8,0.7,1.1,1.0]
	end_save,save_as
end
;------------------------------------------------------------------------------------------
...
pro compare_cci_with_clara, year, month, day, data = data, sat = sat, mini = mini, maxi = maxi 	, $
	...
	endcase
	algo2     = ref2algo(ref)
	algon_cci = sat_name(algo1,sat, year=year, month=month,version=version)
	algon_gac = sat_name(algo2,satgac, year=year, month=month,version=version)
	...
	if is_h1d(dat) and keyword_set(timeseries) then begin
		datum = '200801-200812'
		sav_file = !SAVS_DIR + 'hist1d/'+strlowcase(dat)+'_'+datum+'_'+algo2ref(algo1,sat=sat)+'_'+$
			(total(algo1 eq ['coll5','coll6','claas']) ? '':sat)+'.sav'
		bild_cci = 	restore_var(sav_file,found=found)
		if not found then begin
			ok = dialog_message('compare_cci: sav_file1 not found! '+sav_file)
			return
		endif
		print,'Sav File1: ',sav_file
		sav_file = !SAVS_DIR + 'hist1d/'+strlowcase(dat)+'_'+datum+'_'+algo2ref(algo2,sat=satgac)+'_'+$
			(total(algo2 eq ['coll5','coll6','claas']) ? '':sat)+'.sav'
		bild_gac = restore_var(sav_file,found=found)
		if not found then begin
			ok = dialog_message('compare_cci: sav_file2 not found! '+sav_file)
			return
		endif
		print,'Sav File2: ',sav_file
		fillvalue1 = 0
		fillvalue2 = 0
	...
	if is_h1d(dat) then begin
		if hct eq '1d' then begin
			...
			start_save, save_as5, thick = thick
				...
				plot,[0,0],[1,1],yr=yrange,xr=[0,n_elements(bild_gac)-1],xticks=n_elements(xtickname)-1, $
...
; 				oplot,!x.crange,[regr[0]/bin,regr[1]*!x.crange[1]+regr[0]/bin],linestyle=2
	...
	; cci
	medi_c = zonal_average(dum_bildc,lat,fillvalue=fillvalue1,lat_zon=lat1d,/mean);,/median)
	; gac
	medi_g = zonal_average(dum_bildg,lat,fillvalue=fillvalue2,lat_zon=lat1d,/mean);,/median)
	...
	start_save, save_as4, thick = thick, size = [32, 20]
		...
		str_pholder = strjoin(replicate(' ',max([strlen(sat_name(algo1,sat)),strlen(algon_gac)])))
		print,'-----------'+dat+'-'+cov+'--------------'
		if stregex(dat,'npoints',/fold,/bool) then begin
			print,'Total         '+sat_name(algo1,sat)+' :',string(total(bild_cci[idx]),f='(f15.1)')
			print,'Total         '+algon_gac          +' :',string(total(bild_gac[idx]),f='(f15.1)')
		endif
		print,'Glob. Mean (F1) '+sat_name(algo1,sat)+' : ',string(gmean(bild_cci[idx],lat[idx]),f='(f11.4)')
		...
		print,'Correlation(^2) '+str_pholder        +' : ',string(correlate(bild_cci[idx],bild_gac[idx]),f='(f11.4)') +' ('+$
								   strcompress(string(correlate(bild_cci[idx],bild_gac[idx])^2.,f='(f11.4)'),/rem)+')'
		print,'----------------------------------'
		thick = savbg ? thick + 2 : thick
		idx1  = where(medi_c ne fillvalue1,chk_idx1)
		idx2  = where(medi_g ne fillvalue2,chk_idx2)
		if chk_idx1 gt 0 then oplot,lat1d[idx1],medi_c[idx1],thick=thick,col=cgColor("Red")
		if chk_idx1 gt 0 then oplot,lat1d[idx2],medi_g[idx2],thick=thick
...
; 			endif
	...
	if keyword_set(save_as) then begin
		...
		if strcompress(save_as,/rem) eq '1' then begin
			save_dum = save_dir+prefix+datum+'_'+sat_name(algo,sat)			+ $
	...
	if keyword_set(timeseries) then begin
		if histo1d then begin
			datum = '200801-200812'
			if stregex(dat,'ratio',/fold,/bool) then begin
				sav_file = !SAVS_DIR + 'hist1d/'+strlowcase(strreplace(dat,'ratio','liq',/fold))+'_'+datum+'_'+algo2ref(algo,sat=sat)+'_'+$
					(total(algo eq ['coll5','coll6','claas']) ? '':sat)+'.sav'
				bildl = 	restore_var(sav_file,found=found)
				if not found then begin
					ok = dialog_message('plot_l2: sav_file not found! '+sav_file)
					return
				endif
				sav_file = !SAVS_DIR + 'hist1d/'+strlowcase(strreplace(dat,'ratio','ice',/fold))+'_'+datum+'_'+algo2ref(algo,sat=sat)+'_'+$
					(total(algo eq ['coll5','coll6','claas']) ? '':sat)+'.sav'
				bildi = restore_var(sav_file,found=found)
				if not found then begin
					ok = dialog_message('plot_l2: sav_file not found! '+sav_file)
					return
				endif
				bild  = lonarr([size(bildl,/dim),2])
				bild[*,*,*,0] = temporary(bildl)
				bild[*,*,*,1] = temporary(bildi)
			endif else begin
				sav_file = !SAVS_DIR + 'hist1d/'+strlowcase(dat)+'_'+datum+'_'+algo2ref(algo,sat=sat)+'_'+$
					(total(algo eq ['coll5','coll6','claas']) ? '':sat)+'.sav'
				bild = 	restore_var(sav_file,found=found)
			endelse
			if not found then begin
				ok = dialog_message('plot_l2: sav_file not found! '+sav_file)
				return
			...
			maxvalue = max(bild)
			unit = ''
		endif else if histo2d then begin
			datum = '200801-200812'
			sav_file = !SAVS_DIR + 'cot_ctp_hist2d/'+get_product_name(dat,algo='cci_old')+'_'+datum+'_'+algo2ref(algo,sat=sat)+'_'+$
				(total(algo eq ['coll5','coll6','claas']) ? '':sat)+'.sav'
			bild = 	restore_var(sav_file,found=found)
			if not found then begin
				ok = dialog_message('plot_l2: sav_file not found! '+sav_file)
			...
			if algo eq 'esacci' then begin
				bild = stregex((reverse(strsplit(dat,'_',/ext)))[0],'unc',/fold,/bool) ? d.unc_cci : d.mean_cci
				ii   = stregex((reverse(strsplit(dat,'_',/ext)))[0],'unc',/fold,/bool) ? 9 : 8
			endif else begin
				bild = d.mean_gac
				ii   = 10
			endelse
			...
			fillvalue = -999.
			dumyear  = minmax(where(d.stats[ii,*] ne -999.))/12 + fix((strsplit(d.period,/ext,'-'))[0])
			dummonth = minmax(where(d.stats[ii,*] ne -999.)) mod 12
			datum    = strjoin(string(dumyear,f=('(i4.4)'))+string(dummonth+1,f=('(i2.2)')),'-')
		...
		if hct eq '1d' then begin
			...
			start_save, save_dum, thick = thick
				...
				if ~opl then begin
					...
					xtickname=strcompress(string(xtickname,f='(f20.1)'),/rem), charthick = (savbg ? 2. : 1), $
...
; 			ndims = size(bild,/n_dim)
	...
	make_geo, file = strreplace(file[fidx],'secondary','primary',/fold), lon,lat, verbose = verbose, dimension = size(bild,/dim), $
...
prefix=''
	...
	if keyword_set(limit) then begin
		magnify=1
; 		if img_res ge 0.02 then magnify = 1
; 		if img_res ge 0.1  then magnify = 1
		if img_res ge 0.25 then magnify = 2
		if img_res ge 0.5  then magnify = 5
	endif else if keyword_set(globe) then magnify=0
help,magnify
...
pro compare_l2, file1, file2, data1=data1, data2=data2, mini=mini, maxi=maxi, bin=bin	, $
		...
		msg=msg, logarithmic=logarithmic,timeseries=timeseries,dim3=dim3,addtext=addtext,rot=rot
	...
	satn1  = sat_name(algo1,sat1, year=year, month=month)
	satn2  = sat_name(algo2,sat2, year=year, month=month)
	...
	if keyword_set(timeseries) then begin
		...
		ref  = (algo2ref(algo1) eq 'cci') ? 'gac' : algo2ref(algo1)
		d = get_available_time_series( algo1, dat1, sat1, coverage = coverage, reference = ref, period = zeitraum, sav_file = sav_file, found = found)
		bild1 = (algo2ref(algo1) eq 'cci') ? d.mean_cci : d.mean_gac
		fillvalue1 = -999.
		unit1 = d.unit
	endif else begin
		...
		if hct eq '1d' then begin
			...
			if found1 and found2 and (n_elements(bild1) eq n_elements(bild2)) then begin
				start_save, save_as, thick = thick
					...
					zwi1 = (algo1 eq 'coll6' and stregex(dat,'ctt',/fold,/bool)) ? ' (Day only)' : ''
					zwi2 = (algo2 eq 'coll6' and stregex(dat,'ctt',/fold,/bool)) ? ' (Day only)' : ''
					plot,[0,0],[1,1],yr=yrange,xr=[0,n_elements(bild1)-1],xticks=n_elements(xtickname)-1, $
					xtickname = strcompress(string(xtickname,f='(f20.1)'),/rem), $
...
; 	stdd = stddev(bild1[idx]-bild2[idx]) 
	...
	corr = correlate(bild1[idx],bild2[idx])
...
;------------------------------------------------------------------------------------------
; ----------------------------------------------------------------------------------------------------------------------------------------------
pro gac_ts_plots,hist_data1,hist_data2,mean_data1,mean_data2,ts_data,bin,dat,unit		, $
		 yrange,lines,min_a,max_a,anz,xtickname,qu,ref,log=log,save_as=save_as,sat	, $
		 single_var=single_var,thick=thick,error=error,show_values = show_values	, $
	...
	symsize=n_elements(xtickname) gt 5 ? 1.5 : 1.0
	symsize=(sav or wbg ? 1+symsize : 0.5+symsize)
	datum = keyword_set(datum) and keyword_set(zonal_only) ? datum : ''
	...
	if ~adv_keyword_set(no_compare) then begin
		idx = where(hist_data1 ge 0 and hist_data2 ge 0,idx_cnt)
		if idx_cnt gt 0 then begin
			min_a = yrange[0]
			max_a = yrange[1]
			if ~zoo then begin
				start_save, save_as1, thick = thick, size = [32,20]
					regr = linfit(hist_data1[idx],hist_data2[idx],chisqr=chisqr,prob=prob,sigma=sigma, YFIT=yfit)
					; 2d hist
					if sig and ~sav then pos1 = [0.01,0.05,0.4,0.85]
					aa = hist_2d(hist_data1[idx],hist_data2[idx],bin1=bin,bin2=bin,max1=max_a,max2=max_a,min1=min_a,min2=min_a)
					view2d,aa,xtitle=sat_name('cci',sat)+' '+strupcase(dat)+' '+unit,ytitle=ref+' '+strupcase(dat)+' '+unit,$
...
; 					oplot,!x.crange,[regr[0]/bin,regr[1]*!x.crange[1]+regr[0]/bin],linestyle=2
			...
			if sig or zoo then begin
				start_save, save_as2, thick = thick, size = [32,20]
					if wbg then thick=4
					if ~sav and ~zoo then pos2 = [0.55,0.59,0.95,0.9]
; 					;------zonal-means----------------------
					fillvalue = -999.
					make_geo,lon_d,lat_d,grid=([360.,180.]/size(mean_data1,/dim))[0]
					medi_c = zonal_average(mean_data1,lat_d,fillvalue=fillvalue,lat_zon=lat1d_c,/mean)
					medi_g = zonal_average(mean_data2,lat_d,fillvalue=fillvalue,lat_zon=lat1d_c,/mean)
					yr  = [(dat eq 'CTT' ? 200:0.1),max([medi_c,medi_g])] 
					; entweder normal
; 					bias = mean(ts_data[2,where(ts_data[0,*] ne -999)])
; 					rmse = mean(ts_data[3,where(ts_data[0,*] ne -999)])
; 					stdd = mean(ts_data[4,where(ts_data[0,*] ne -999)])
					; oder globale bias, rmse und bc-rmse gewichtet nach latitude
					bias = mean(ts_data[6,where(ts_data[0,*] ne -999)])
					rmse = mean(ts_data[7,where(ts_data[0,*] ne -999)])
					stdd = sqrt((rmse^2. - bias^2.) > 0); bc_rmse
					plot,[0,0],[1,1],xr=[-90,90],xs=3,xticks=6,xtickname=['-90','-60','-30','0','30','60','90'], $
					xtitle='latitude [degrees]',ytitle='zonal mean '+dat+unit,yr=yr,position=pos2,noerase=~zoo,ylog=log,$
					title= 	'bias: '+string(bias,f='(f7.2)')+' ; rmse: '+string(rmse,f='(f6.2)')+' ; stdd: '+string(stdd,f='(f6.2)')+unit
					idx1 = where(medi_c ne fillvalue,chk_idx1)
					idx2 = where(medi_g ne fillvalue,chk_idx2)
					if chk_idx1 gt 0 then oplot,lat1d_c[idx1],medi_c[idx1],thick=thick,col=cgColor("Red")
					if chk_idx1 gt 0 then oplot,lat1d_c[idx2],medi_g[idx2],thick=thick
					if keyword_set(show_values) then begin
						legend,[datum+' '+ref,datum+' '+sat_name('cci',sat)],thick=replicate(thick,2),spos='top',charsize=1.5,color=[-1,cgColor("Red")]
					endif else begin
						legend,datum+' '+ref,thick=thick,color=-1,spos='bl',charsize=1.5
						legend,datum+' '+sat_name('cci',sat),thick=thick,color=cgColor("Red"),spos='br',charsize=1.5
					endelse
					;--------------------------------------------
				end_save,save_as2
				if zoo then return
		...
		start_save, save_as3, thick = thick, size = [32,20]
			plot,[0,0],[1,1],xr=[anz[0],anz[1]],xs=3,xticks=n_elements(xtickname)-1,xtickname=xtickname,yr=yrange,ys=9,xticklen=0.01,ytitle=dat+' '+unit,$
			title=title,xminor=xminor, ylog = log, $
			...
			idx = where(ts_data[0,*] gt 0 and ts_data[1,*] gt 0,idx_cnt)
			if idx_cnt eq 0 then return 
			split = where((idx[1:*]-idx[0:*]) ne 1,count)
			if count ge 1 then begin
				if keyword_set(error) and mean(ts_data[5,*]) ne -999. then begin
					polyfill,[idx[0:split[0]]+0.5,reverse(idx[0:split[0]]+0.5)],[reform(ts_data[0,idx[0:split[0]]]+ts_data[5,idx[0:split[0]]]), $
					reverse(reform(ts_data[0,idx[0:split[0]]]-ts_data[5,idx[0:split[0]]]))],col=cgColor("Tan6")
				endif
				oplot,idx[0:split[0]]+0.5,ts_data[0,idx[0:split[0]]],psym=-8,col=cgColor("Red"),thick=thick,symsize=symsize
				oplot,idx[0:split[0]]+0.5,ts_data[1,idx[0:split[0]]],psym=-8,thick=thick,symsize=symsize
				oplot,idx[0:split[0]]+0.5,ts_data[4,idx[0:split[0]]]*qu,psym=-8,col=cgColor("Slate Gray"),thick=thick,symsize=symsize
				if count gt 1 then begin
					for i=1,count-1 do begin
						if keyword_set(error) and mean(ts_data[5,*]) ne -999. then begin
							polyfill,[idx[(split[i-1]+1):split[i]]+0.5,reverse(idx[(split[i-1]+1):split[i]]+0.5)],$
							[reform(ts_data[0,idx[(split[i-1]+1):split[i]]]+ts_data[5,idx[(split[i-1]+1):split[i]]]), $
							reverse(reform(ts_data[0,idx[(split[i-1]+1):split[i]]]-ts_data[5,idx[(split[i-1]+1):split[i]]]))],col=cgColor("Tan6")
						endif
						oplot,idx[(split[i-1]+1):split[i]]+0.5,ts_data[0,idx[(split[i-1]+1):split[i]]],psym=-8,col=cgColor("Red"),thick=thick,symsize=symsize
						oplot,idx[(split[i-1]+1):split[i]]+0.5,ts_data[1,idx[(split[i-1]+1):split[i]]],psym=-8,thick=thick,symsize=symsize
						oplot,idx[(split[i-1]+1):split[i]]+0.5,ts_data[4,idx[(split[i-1]+1):split[i]]]*qu,psym=-8,col=cgColor("Slate Gray"),thick=thick,symsize=symsize
					endfor
				endif
				if keyword_set(error) and mean(ts_data[5,*]) ne -999. then begin
					polyfill,[idx[split[count-1]+1:*]+0.5,reverse(idx[split[count-1]+1:*]+0.5)],$
					[reform(ts_data[0,idx[split[count-1]+1:*]]+ts_data[5,idx[split[count-1]+1:*]]),$
					reverse(reform(ts_data[0,idx[split[count-1]+1:*]]-ts_data[5,idx[split[count-1]+1:*]]))],col=cgColor("Tan6")
				endif
				oplot,idx[split[count-1]+1:*]+0.5,ts_data[0,idx[split[count-1]+1:*]],psym=-8,col=cgColor("Red"),thick=thick,symsize=symsize
				oplot,idx[split[count-1]+1:*]+0.5,ts_data[1,idx[split[count-1]+1:*]],psym=-8,thick=thick,symsize=symsize
				oplot,idx[split[count-1]+1:*]+0.5,ts_data[4,idx[split[count-1]+1:*]]*qu,psym=-8,col=cgColor("Slate Gray"),thick=thick,symsize=symsize
			endif else begin
				if keyword_set(error) and mean(ts_data[5,*]) ne -999. then begin
					polyfill,[idx,reverse(idx)]+0.5,[reform(ts_data[0,idx]+ts_data[5,idx]),reverse(reform(ts_data[0,idx]-ts_data[5,idx]))],col=cgColor("Tan6")
				endif
				oplot,idx+0.5,ts_data[0,idx],psym=-8,col=cgColor("Red"),thick=thick,symsize=symsize
				oplot,idx+0.5,ts_data[1,idx],psym=-8,thick=thick,symsize=symsize
				oplot,idx+0.5,ts_data[4,idx]*qu,psym=-8,col=cgColor("Slate Gray"),thick=thick,symsize=symsize
			endelse
			legend,ref,psym=-8,thick=thick,color=-1,spos='tl',charsize=lcharsize,charthick=charthick
			legend,sat_name('cci',sat),psym=-8,thick=thick,color=[cgColor("Red")],spos='tr',charsize=lcharsize,charthick=charthick
			legend,'STDDEV',psym=-8,thick=thick,color=[cgColor("Slate Grey")],spos='top',charsize=lcharsize,charthick=charthick
			axis,yaxis=1,ystyle=1,yrange=yrange/qu,col = cgColor("Slate Gray"),ytitle='STD'+' '+unit, $
			charthick = charthick, xcharsize = xcharsize, ycharsize= ycharsize
		end_save,save_as3
	endif

	if adv_keyword_set(no_compare) then begin
		nc     = no_compare eq 0 ? 8 : 10; no_compare 0 für cci und 1 für andere ; hier werden jetzt andere indexe genommen weil nicht verglichen wird 
		nc_unc = 9
		if keyword_set(standard) then nc = no_compare eq 0 ? 11 : 12
		start_save, save_as3, thick = thick, size = [32,20]
			if wbg then thick=4
			idx = where(ts_data[nc,*] gt 0,idx_cnt)
			if idx_cnt eq 0 then return
			split = where((idx[1:*]-idx[0:*]) ne 1,count)
			if ~opl then begin
				...
				xmargin=[10,10]+(sav or wbg ? [(wbg ? 10:4),0]:0),ymargin=[5,2]+(sav or wbg ? [2,1]:0),$
				charthick = charthick, xcharsize = xcharsize, ycharsize = ycharsize
				div = ((anz[1]-anz[0]) le 12 ? 4. : 12.)
				for i = 0, (anz[1]-anz[0])/div do oplot,anz[0]+[i*div,i*div],!y.crange,linestyle=2
				for i = 0, n_elements(lines)-1 do oplot,!x.crange,[lines[i],lines[i]],linestyle=1
				if count ge 1 then begin
					if keyword_set(error) and nc eq 8 and mean(ts_data[nc_unc,*]) ne -999. then begin
						polyfill,[idx[0:split[0]]+0.5,reverse(idx[0:split[0]]+0.5)],[reform(ts_data[nc,idx[0:split[0]]]+ts_data[nc_unc,idx[0:split[0]]]), $
						reverse(reform(ts_data[nc,idx[0:split[0]]]-ts_data[nc_unc,idx[0:split[0]]]))],col=cgColor("Tan6")
					endif
					oplot,idx[0:split[0]]+0.5,ts_data[nc,idx[0:split[0]]],psym=-8,thick=thick,symsize=symsize
					if count gt 1 then begin
						for i=1,count-1 do begin
							if keyword_set(error) and nc eq 8 and mean(ts_data[nc_unc,*]) ne -999. then begin
								polyfill,[idx[(split[i-1]+1):split[i]]+0.5,reverse(idx[(split[i-1]+1):split[i]]+0.5)],$
								[reform(ts_data[nc,idx[(split[i-1]+1):split[i]]]+ts_data[nc_unc,idx[(split[i-1]+1):split[i]]]), $
								reverse(reform(ts_data[nc,idx[(split[i-1]+1):split[i]]]-ts_data[nc_unc,idx[(split[i-1]+1):split[i]]]))],col=cgColor("Tan6")
							endif
							oplot,idx[(split[i-1]+1):split[i]]+0.5,ts_data[nc,idx[(split[i-1]+1):split[i]]],psym=-8,thick=thick,symsize=symsize
						endfor
					endif
					if keyword_set(error) and nc eq 8 and mean(ts_data[nc_unc,*]) ne -999. then begin
						polyfill,[idx[split[count-1]+1:*]+0.5,reverse(idx[split[count-1]+1:*]+0.5)],$
						[reform(ts_data[nc,idx[split[count-1]+1:*]]+ts_data[nc_unc,idx[split[count-1]+1:*]]),$
						reverse(reform(ts_data[nc,idx[split[count-1]+1:*]]-ts_data[nc_unc,idx[split[count-1]+1:*]]))],col=cgColor("Tan6")
					endif
					oplot,idx[split[count-1]+1:*]+0.5,ts_data[nc,idx[split[count-1]+1:*]],psym=-8,thick=thick,symsize=symsize
				endif else begin
					if keyword_set(error) and nc eq 8 and mean(ts_data[nc_unc,*]) ne -999. then begin
						polyfill,[idx,reverse(idx)]+0.5,[reform(ts_data[nc,idx]+ts_data[nc_unc,idx]),reverse(reform(ts_data[nc,idx]-ts_data[nc_unc,idx]))],col=cgColor("Tan6")
					endif
					oplot,idx+0.5,ts_data[nc,idx],psym=-8,thick=thick,symsize=symsize
				endelse
				legend,ref+hct,psym=-8,thick=thick,color=-1,spos='top',charsize=lcharsize,charthick=charthick
			endif else begin
				...
				if count ge 1 then begin
					...
					oplot,idx[0:split[0]]+0.5,ts_data[nc,idx[0:split[0]]],psym=psym,thick=thick,col=cgcolor(cols),linestyle=linestyle,symsize=symsize
					if count gt 1 then begin
						for i=1,count-1 do begin
							if keyword_set(error) and nc eq 8 and mean(ts_data[nc_unc,*]) ne -999. then begin
								polyfill,[idx[(split[i-1]+1):split[i]]+0.5,reverse(idx[(split[i-1]+1):split[i]]+0.5)],$
								[reform(ts_data[nc,idx[(split[i-1]+1):split[i]]]+ts_data[nc_unc,idx[(split[i-1]+1):split[i]]]), $
								reverse(reform(ts_data[nc,idx[(split[i-1]+1):split[i]]]-ts_data[nc_unc,idx[(split[i-1]+1):split[i]]]))],$
								col=cgColor("Tan6")
							endif
							oplot,idx[(split[i-1]+1):split[i]]+0.5,ts_data[nc,idx[(split[i-1]+1):split[i]]],psym=psym,thick=thick,col=cgcolor(cols),$
							linestyle=linestyle,symsize=symsize
						endfor
					endif
					if keyword_set(error) and nc eq 8 and mean(ts_data[nc_unc,*]) ne -999. then begin
						polyfill,[idx[split[count-1]+1:*]+0.5,reverse(idx[split[count-1]+1:*]+0.5)],$
					...
					oplot,idx[split[count-1]+1:*]+0.5,ts_data[nc,idx[split[count-1]+1:*]],psym=psym,thick=thick,col=cgcolor(cols),linestyle=linestyle,symsize=symsize
				endif else begin
					if keyword_set(error) and nc eq 8 and mean(ts_data[nc_unc,*]) ne -999. then begin
						polyfill,[idx,reverse(idx)]+0.5,[reform(ts_data[nc,idx]+ts_data[nc_unc,idx]),reverse(reform(ts_data[nc,idx]-ts_data[nc_unc,idx]))],$
						col=cgColor("Tan6")
					endif
					oplot,idx+0.5,ts_data[nc,idx],psym=psym,thick=thick,col=cgcolor(cols),linestyle=linestyle,symsize=symsize
				endelse
				legend,ref+hct,thick=thick,color=cgcolor(cols),spos=spos,ystretch=ystretch*(opl le 2 ? 1.3 : 1.1),$
...
; ----------------------------------------------------------------------------------------------------------------------------------------------
pro plot_cci_gac_time_series, 	diff = diff, sat = sat, reference = reference, save_as=save_as,win_nr=win_nr,mean_2d=mean_2d		, $
				...
				msg=msg, logarithmic=logarithmic,white_bg=white_bg
	...
	datum = '2008-2008' ; start dummy

	algon = sat_name(ref,sat)
	set_colors,rainbow,bwr,extended_rainbow,greyscale,elevation,flip_colours,other=other,ctable=ctable,brewer=brewer,col_tab=col_tab

	if keyword_set(single_var) then begin
		single = strlowcase(single_var)
		if single eq 'cc_total' then single = 'cfc'
		if not total(['cot','ctp','ctt','lwp','iwp','cfc','cwp','ref','cph'] eq single[0]) then begin
			ok = dialog_message("Choose one of these 'cot','ctp','ctt','lwp','iwp','cfc','cc_total','cwp','ref','cph'")
			return
		endif
	endif

	d = get_available_time_series( 	'esacci', single, sat, coverage = coverage, reference = ref, period = datum, $
					sav_file = sfile, longname = longname, unit = unit, found = found)
	...
	dumyear  = minmax(where(d.stats[0,*] ne -999.))/12 + fix((strsplit(d.period,/ext,'-'))[0])
	dummonth = minmax(where(d.stats[0,*] ne -999.)) mod 12
	datum    = strjoin(string(dumyear,f=('(i4.4)'))+string(dummonth+1,f=('(i2.2)')),'-')+' '

	if keyword_set(save_as) then begin 
		save_as = !SAVE_DIR +'timeseries'+'/'+cci_name(sat)+'_vs_'+algon+'_time_series_'+$
	...
	make_geo,lon,lat,grid = get_grid_res(d.MEAN_CCI)

	if keyword_set(diff) then begin
		start_save, save_as, thick = thick, size = keyword_set(single_var) ? [32,20] :'A3'
			dumdata = d.mean_cci - d.mean_gac & minv = -20. & maxv = 20.
			ititle = datum+' '+longname & btitle = 'Diff '+sat_name('cci',sat)+' - '+algon+' '+strupcase(single)+unit
			m = obj_new("map_image",dumdata,lat,lon,void_index=where(d.mean_cci eq -999.),n_lev=4	, $
	...
	if keyword_set(mean_2d) then begin
		start_save, save_as, thick = thick, size = keyword_set(single_var) ? [32,20] :'A3'
			!p.multi= (keyword_set(single_var) ? [0,2,1] : [0,2,6])
			dumdata = d.mean_cci & minv = 0. & maxv = 100.
			ititle = datum+' '+longname & btitle = sat_name('cci',sat)+' '+strupcase(single)+unit
			...
			obj_destroy,m
			dumdata = d.mean_gac & minv = 0. & maxv = 100.
			ititle = datum+' '+longname & btitle = algon+' '+strupcase(single)+unit
			m = obj_new("map_image",dumdata,lat,lon,void_index=where(dumdata eq -999.),n_lev=4	, $
				max=(adv_keyword_set(maxi) ? maxi : maxv),min=(adv_keyword_set(mini) ? mini: minv)	, $
				countries=countries,magnify = magnify, figure_title = ititle, title= btitle		, $
				charthick = keyword_set(save_as) ? 2. : 1.5, charsize = (keyword_set(save_as) ? 3. : 1.5)	, $
	...
	log = 0
	anz_yy = 3
	anz = anz_yy * 12 ; monate 
	if anz_yy eq 3 then xtickname=['2007',replicate(' ',11),'2008',replicate(' ',11),'2009',replicate(' ',11),'2010'] $
	else xtickname = (string(indgen(anz_yy+1)+1982,f='(i4.4)'))[0:*:2]

	start_save, save_as, thick = thick, size = keyword_set(single_var) ? [32,20] :'A3'
		plot_simple_timeseries, '2007','01', single_var[0], sat, 'esacci', cov, reference = ref, structure=d,zonal_only=zonal_only, $
...
pro vergleiche_ctp_cot_histogram_cci_mit_clara, ccifile, varname = varname, mini = mini, maxi = maxi, limit=limit, zoom=zoom,algo1 = algo1, sat=sat,$
	...
	data  = keyword_set(varname) ? strlowcase(varname) : 'cot_ctp_hist2d'
	...
	case ref of 
		'cci'	: begin & algo = 'esacci' & algon = sat_name(algo,sat) & end
		'cci_old' : begin & algo = 'esacci_old' & algon = sat_name(algo,sat) & end
		'gac'	: begin & algo = 'clara'  & algon = sat_name(algo,sat) & end
		'gac2'	: begin & algo = 'clara2' & algon = sat_name(algo,sat) & end
		'myd'	: begin & algo = 'coll5'  & algon = 'Coll5-AQUA' & end
		'mod'	: begin & algo = 'coll5'  & algon = 'Coll5-TERRA' & end
		'myd2'	: begin & algo = 'coll6'  & algon = 'Coll6-AQUA' & end
		'mod2'	: begin & algo = 'coll6'  & algon = 'Coll6-TERRA' & end
		'gwx'	: begin & algo = 'gewex'  & algon = sat_name(algo,sat) & end
		'cal'	: begin & algo = 'calipso' & algon = sat_name(algo,sat) & end
		'isp'	: begin & algo = 'isccp'  & algon = sat_name(algo,sat) & end
		'pmx'	: begin & algo = 'patmos' & algon = sat_name(algo,sat) & end
		'cla'	: begin & algo = 'claas'  & algon = sat_name(algo,sat) & end
		'self'	: begin & ok = dialog_message('vergleiche_ctp_cot_histogram_cci_mit_clara: Choose Reference Dataset') & return & end
		else	:
	endcase
	...
	if keyword_set(timeseries) then begin
		datum = '200701-200912'
		cci = restore_var(!SAVS_DIR + 'cot_ctp_hist2d/cot_ctp_hist2d_'+datum+'_'+algo2ref(algo1,sat=sat)+'_'+sat+'.sav',found=found)
		if not found then begin
			ok = dialog_message('No time_series "sav" file found for '+sat_name(algo1,sat))
			return
		endif
		gac = restore_var(!SAVS_DIR + 'cot_ctp_hist2d/cot_ctp_hist2d_'+datum+'_'+ref+'_'+(strmid(ref2algo(ref),0,4) eq 'coll' ? '':sat)+'.sav',found=found)
		if not found then begin
			ok = dialog_message('vergleiche_ctp_cot_histogram_cci_mit_clara: No time_series "sav" file found for '+sat_name(ref,sat))
			return
		endif
		fillvalue = -999.
	endif else begin
	...
	if hct eq '1d' then begin
		...
		if keyword_set(save_as)   then begin
			!p.multi=0
			save_dum = save_dir+'ctp_cot_histogram_'+datum+'_'+$
		...
		if is_jch(varname,/rat) then begin
			...
			cci_histos = {	cot:(reform(float(liq.cot)/float((liq.cot>0)+(ice.cot>0)>1)) *100.),$
					ctp:(reform(float(liq.ctp)/float((liq.ctp>0)+(ice.ctp>0)>1)) *100.)}
			liq = get_1d_hist_from_jch(gac[*,*,*,*,0],algo,bin_name=xtickname,limit=limit,antarctic=antarctic,arctic=arctic, $
						land=land, sea=sea,found=found2)
			ice = get_1d_hist_from_jch(gac[*,*,*,*,1],algo,bin_name=xtickname,limit=limit,antarctic=antarctic,arctic=arctic, $
						land=land, sea=sea,found=found2)
			...
			cci_histos = get_1d_hist_from_jch(cci,algo1,bin_name=xtickname,limit=limit,antarctic=antarctic,arctic=arctic, $
						land=land, sea=sea,found=found1)
			gac_histos = get_1d_hist_from_jch(gac,algo,bin_name=xtickname,limit=limit,antarctic=antarctic,arctic=arctic, $
						land=land, sea=sea,found=found2)
		...
		if found1 and found2 then begin
			apx = stregex(data,'ice',/fold,/bool) ? 'Ice ' : (stregex(data,'liq',/fold,/bool) ? 'Liquid ' : '')
			; CTP
			if keyword_set(save_as) then start_save, save_dum+'_'+sat_name(algo1,sat)+'_-_'+algon+'_CTP.eps', thick = thick else thick = 2
				plot,[0,0],[1,1],yr=yrange,xr=[0,6],xticks=7,xtickname=xtickname.ctp, $
				...
				legend,[sat_name(algo1,sat),algon],psym=[8,8],numsym=1,color=[cgColor('Red'),-1],$
				thick=replicate(thick,2),clrbox=clrbox,spos='tr', charsize=(keyword_set(save_as) ? 2.5:1.5)
			if keyword_set(save_as) then end_save, save_dum+'_'+sat_name(algo1,sat)+'_-_'+algon+'_CTP.eps'
			; COT
			if keyword_set(save_as) then start_save, save_dum+'_'+sat_name(algo1,sat)+'_-_'+algon+'_COT.eps', thick = thick else thick = 2
				plot,[0,0],[1,1],yr=yrange,xr=[0,5],xticks=6,xtickname=xtickname.cot, $
				...
				legend,[sat_name(algo1,sat),algon],psym=[8,8],numsym=1,color=[cgColor('Red'),-1],$
				thick=replicate(thick,2),clrbox=clrbox,spos='tr', charsize=(keyword_set(save_as) ? 2.5:1.5)
			if keyword_set(save_as) then end_save, save_dum+'_'+sat_name(algo1,sat)+'_-_'+algon+'_COT.eps'
		endif
		return
	endif
	...
	if not found then stop ; should not happen, except an unknown regular grid resolution is used 
	...
	if hct eq 'max' then begin
		; -------maxtype
		gac = get_hct_maxtype( gac, algo, grid_res = out_grid,lon=lon,lat=lat,fillvalue=fillvalue, htypes=htypes)
		; -------
	endif else if hct eq 'hist2d' or hct eq 'hist_2d' then begin
		gac = get_2d_rel_hist_from_jch( gac, algo, dem = dem, land = land, sea = sea, limit = limit, antarctic = antarctic, arctic = arctic, $
						lon = lon, lat = lat, fillvalue = fillvalue, found = found)
		if not found then return
		if keyword_set(save_as) then begin
			!p.multi=0
...
; 			if ~keyword_set(difference) then !p.multi=[0,2,1]
		endelse
		plot_2d_rel_hist, cci, sat_name(algo1,sat), bild2=gac, name2=algon, col_tab=col_tab, brewer=brewer, $
		mini=mini, maxi=maxi, save_as=save_dum, difference = difference
		return
	endif else begin
		if is_jch(varname,/rat) then begin
			liq = get_hct_data(hct,gac[*,*,*,*,0],algo,sdum=sdum,found=found,grid=out_grid)
			ice = get_hct_data(hct,gac[*,*,*,*,1],algo,sdum=sdum,found=found,grid=out_grid)
			gac = reform(float(liq)/float((liq>0)+(ice>0)>1)) *100.
			bar_title = 'Liquid Fraction '+(keyword_set(hct) ? 'of '+strupcase(hct)+' Clouds [%]':'')
		endif else begin
			gac = get_hct_data(hct,gac,algo,/relative,found=found,sdum=sdum,grid=out_grid)
	...
	if hct ne 'max' and ~is_jch(varname,/rat) then begin
		print,strupcase(hct)+'/all Ratio '+sat_name(algo1,sat)+' : '+ratio_cci;rat_cci+'%'
		print,strupcase(hct)+'/all Ratio '+algon+' : '+ratio_gac;rat_gac+'%'
	endif
	minvalue = 0
	maxvalue = 1
	if ls then begin
	...
	if hct eq 'max' then begin
		if keyword_set(save_as) then save_as = save_dum+'_CCI_'+strupcase(sat)+'_max_type.eps'
		start_save, save_as, thick = thick,size=[32,20]
			m = obj_new("map_image",cci,lat_c,lon_c,void_index=void_index1,box_axes=box_axes,n_lev=9, $
				...
				figure_title = sat_name(algo1,sat)+' max type', charthick = keyword_set(save_as) ? 2. : 1.5	, $
				title= 'Cloud type',$
			...
			obj_destroy,m
		end_save, save_as
		if keyword_set(save_as) then save_as = save_dum+'_'+algon+'_max_type.eps'
		start_save, save_as, thick = thick,size=[32,20]
			m = obj_new("map_image",gac,lat_g,lon_g,void_index=void_index2,box_axes=box_axes,n_lev=9, $
				magnify=magnify, min=0,max=9,ctable = 13,discrete=findgen(10),$
				figure_title = algon+' max type', charthick = keyword_set(save_as) ? 2. : 1.5	, $
				title= 'Cloud type',$
				bar_tickname= [htypes], logarithmic=logarithmic,label=label,horizon=horizon, $
				format=bar_format, charsize = (keyword_set(save_as) ? 3. : 1.5), $
				p0lon = p0lon, p0lat = p0lat,lambert=lambert	, $
		...
		if ~keyword_set(difference) then begin
			if keyword_set(save_as) then save_as = save_dum+'_CCI_'+strupcase(sat)+'_Ratio_'+strcompress(rat_cci,/rem)+'.eps'
			start_save, save_as, thick = thick,size=[20,16]
				m = obj_new("map_image",cci,lat_c,lon_c,void_index=void_index1,box_axes=box_axes,n_lev=5, $
					...
					figure_title = sat_name(algo1,sat)+' '+lname, charthick = keyword_set(save_as) ? 2. : 1.5	, $
			...
			if keyword_set(save_as) then save_as = save_dum+'_'+algon+'_Ratio_'+strcompress(rat_gac,/rem)+'.eps'
			start_save, save_as, thick = thick,size=[20,16]
				m = obj_new("map_image",gac,lat_g,lon_g,void_index=void_index2,box_axes=box_axes,n_lev=5, $
					max=adv_keyword_set(maxi) ? maxi:maxvalue, min=adv_keyword_set(mini) ? mini:minvalue, magnify=magnify, $
					figure_title = algon+' '+lname, charthick = keyword_set(save_as) ? 2. : 1.5	, $
					title= keyword_set(bar_title) ? bar_title :'Rel. Occ. '+(keyword_set(hct) ? 'of '+strupcase(hct)+' Clouds [%]' : ''),$
					format=bar_format, charsize = (keyword_set(save_as) ? 3. : 1.5), logarithmic=logarithmic, $
					bwr = bwr, elevation = elevation, extended_rainbow = extended_rainbow	, flip_colours = flip_colours,$
			...
			; 2d histogram
			if keyword_set(save_as) then save_as = save_dum+'_CCI_'+strupcase(sat)+'_'+algon+'_2dHist.eps'
			start_save, save_as, thick = thick,size=[20,16]
				...
				xtitle=sat_name(algo1,sat),ytitle=algon
		...
		;difference
		if keyword_set(save_as) then save_as = save_dum+'_CCI_'+strupcase(sat)+'-'+algon+'_Diff.eps'
		start_save, save_as, thick = thick,size=[20,16]
			...
			m = obj_new("map_image",(cci-gac),lat_g,lon_g,void_index=void_index,box_axes=box_axes,n_lev=5, $
				...
				figure_title = 'Diff. '+sat_name(algo1,sat)+' - '+algon+' '+lname, charthick = keyword_set(save_as) ? 2. : 1.5	, $
				title= keyword_set(bar_title) ? bar_title:'Rel. Occ. '+(keyword_set(hct) ? 'of '+strupcase(hct)+' Clouds [%]' : ''),$
				format=bar_format, charsize = (keyword_set(save_as) ? 3. : 1.5), logarithmic=logarithmic, $
	...
	if keyword_set(zoom) then begin
		vergleiche_ctp_cot_histogram_cci_mit_clara, ccifile,  mini = mini, maxi = maxi, limit=zoom,algo = algo, sat=sat,$
...
pro make_2d_overview,year=year,month=month,satellite,reference=reference, coverage = coverage, algo = algo, out = out,addtext=addtext, $
	...
	sat1 = keyword_set(satellite) ? strlowcase(satellite) : 'noaa18'
	ref  = keyword_set(reference) ? strlowcase(reference) : 'gac'
	cov  = keyword_set(coverage)  ? strlowcase(coverage)  : 'full'
...
;---------
	case cov of
		...
		'antarctica'	: dem[where(between(lat,-90.0,-65.5))] = 9999.
		'midlat_south'	: dem[where(between(lat,-65.5,-23.5))] = 9999.
		'tropic'	: dem[where(between(lat,-23.5, 23.5))] = 9999.
		'midlat_north'	: dem[where(between(lat, 23.5, 65.5))] = 9999.
		'arctic'	: dem[where(between(lat, 65.5, 90.0))] = 9999.
	...
	if ~keyword_set(ref) then return
	...
	algon1  = sat_name(algo,sat1)
	algo2   = ref
	algon2  = sat_name(ref,sat2)
	dummy   = fltarr(360,180) -999.

	if keyword_set(time_series) then begin
		dum = restore_var(!SAVS_DIR + 'overview_matrix/two_dim_overview_matrix_200701-200912_'+algo1+'_'+sat1+'.sav',found=found)
		if found then gesamt_cci = dum.mean else begin
			ok = dialog_message('make_2d_overview: Time Series Sav not found for '+algo1+' '+sat1)
			return
		endelse
		dum = restore_var(!SAVS_DIR + 'overview_matrix/two_dim_overview_matrix_200701-200912_'+algo2+'_'+sat2+'.sav')
		...
		ctt     = get_data(file=file1,year,month,data='ctt',algo=algo1,sat=sat1,level='l3c',/glob,no_data_value=no_data_value,/mean,/make_comp,/sil,found = found)
 		if ~found then ctt = dummy; else begin
; 			idx = where(ctt eq no_data_value[0],idx_cnt)
; 			ctt = ctt - 150
; 			if idx_cnt gt 0 then ctt[idx] = no_data_value[0]
; 		endelse
		ctp     = get_data(file=file1,year,month,data='ctp',algo=algo1,sat=sat1,level='l3c',/glob,no_data_value=no_data_value,/mean,/make_comp,/sil,found = found)
		if ~found then ctp = dummy
		cfc     = get_data(file=file1,year,month,data='cfc',algo=algo1,sat=sat1,level='l3c',/glob,no_data_value=no_data_value,/mean,/make_comp,/sil,found = found)
		if ~found then cfc = dummy
		cot     = get_data(file=file1,year,month,data='cot',algo=algo1,sat=sat1,level='l3c',/glob,no_data_value=no_data_value,/mean,/make_comp,/sil,found = found)
		if ~found then cot = dummy
		cph     = get_data(file=file1,year,month,data='cph',algo=algo1,sat=sat1,level='l3c',/glob,no_data_value=no_data_value,/mean,/make_comp,/sil,found = found)
		if ~found then cph = dummy
		ref     = get_data(file=file1,year,month,data='ref',algo=algo1,sat=sat1,level='l3c',/glob,no_data_value=no_data_value,/mean,/make_comp,/sil,found = found)
		if ~found then ref = dummy
		lwp     = get_data(file=file1,year,month,data='lwp',algo=algo1,sat=sat1,level='l3c',/glob,no_data_value=no_data_value,/mean,/make_comp,/sil,found = found)
		if ~found then lwp = dummy
		iwp     = get_data(file=file1,year,month,data='iwp',algo=algo1,sat=sat1,level='l3c',/glob,no_data_value=no_data_value,/mean,/make_comp,/sil,found = found)
		if ~found then iwp = dummy
		cwp     = get_data(file=file1,year,month,data='cwp',algo=algo1,sat=sat1,level='l3c',/glob,no_data_value=no_data_value,/mean,/make_comp,/sil,found = found)
		if ~found then cwp = dummy

		ctt_ref = get_data(file=file2,year,month,data='ctt',algo=algo2,sat=sat2,level='l3c',/glob,no_data_value=no_data_value,/mean,/make_comp,/sil,found=found)
 		if ~found then ctt_ref = dummy ;else begin
; 			idx = where(ctt_ref eq no_data_value[0],idx_cnt)
; 			ctt_ref = ctt_ref - 150
; 			if idx_cnt gt 0 then ctt_ref[idx] = no_data_value[0]
; 		endelse
		ctp_ref = get_data(file=file2,year,month,data='ctp',algo=algo2,sat=sat2,level='l3c',/glob,no_data_value=no_data_value,/mean,/make_comp,/sil,found=found)
		if ~found then ctp_ref = dummy
		cfc_ref = get_data(file=file2,year,month,data='cfc',algo=algo2,sat=sat2,level='l3c',/glob,no_data_value=no_data_value,/mean,/make_comp,/sil,found=found)
		if ~found then cfc_ref = dummy
		cot_ref = get_data(file=file2,year,month,data='cot',algo=algo2,sat=sat2,level='l3c',/glob,no_data_value=no_data_value,/mean,/make_comp,/sil,found=found)
		if ~found then cot_ref = dummy
		cph_ref = get_data(file=file2,year,month,data='cph',algo=algo2,sat=sat2,level='l3c',/glob,no_data_value=no_data_value,/mean,/make_comp,/sil,found=found)
		if ~found then cph_ref = dummy
		ref_ref = get_data(file=file2,year,month,data='ref',algo=algo2,sat=sat2,level='l3c',/glob,no_data_value=no_data_value,/mean,/make_comp,/sil,found=found)
		if ~found then ref_ref = dummy
		lwp_ref = get_data(file=file2,year,month,data='lwp',algo=algo2,sat=sat2,level='l3c',/glob,no_data_value=no_data_value,/mean,/make_comp,/sil,found=found)
		if ~found then lwp_ref = dummy
		iwp_ref = get_data(file=file2,year,month,data='iwp',algo=algo2,sat=sat2,level='l3c',/glob,no_data_value=no_data_value,/mean,/make_comp,/sil,found=found)
		if ~found then iwp_ref = dummy
		cwp_ref = get_data(file=file2,year,month,data='cwp',algo=algo2,sat=sat2,level='l3c',/glob,no_data_value=no_data_value,/mean,/make_comp,/sil,found=found)
...
; 		out = (gesamt_cci-gesamt_ref)/(gesamt_cci+gesamt_ref) 	; werte maximal 1 : Ref = 0 und CCI != 0, minimal -1 : CCI = 0 und Ref !=0, 0 ist perfekt.
		...
		col_tab=4,no_data_idx=where(gesamt_ref le 0. or gesamt_cci le 0. or dem ne 9999.),charsize=1.5,xmargin=[8,9],ymargin=[3,3],$;xmargin=[8,8]$
...
pro plot_hovmoeller, data, algo, satellite, save_as = save_as, mini = mini, maxi = maxi, win_nr=win_nr,$
	...
	if keyword_set(save_as) then begin
		save_as = !SAVE_DIR +'hovmoeller/'+(keyword_set(ref) ? 'Diff_'+algon+'_-_'+ref_name:algon)+'_'+dat+'_hovmoeller_L3C_2007-2009' $
				    +(keyword_set(land)?'_land':'')+(keyword_set(sea)?'_sea':'')+'.eps'
		!p.multi=0
	...
	start_save, save_as
	make_cool_contour,matrix,findgen(si[0]),findgen(si[1]),nlev,/lines, charsize=2,xmargin=[8,3],ymargin=[5,2], $
		title=title,bar_title=strupcase(dat),no_data_val=no_data_val, $
		ytickname=ytickname,yticks=n_elements(ytickname)-1,ytitle='Latitude',c_charsize = 2., $
		xtickname=xtickname,xticks=n_elements(xtickname)-1,min=mima[0],max=mima[1],color=0,$
		...
		for j = 1,5 do oplot,!x.crange,[j*30,j*30],linestyle=1
		for j = 1, si[0]/12 -1 do oplot,j*[12.,12.],!y.crange,linestyle=2
	end_save, save_as
end
;-------------------------------------------------------------------------------------------------------------------------
pro plot_histogram,year,month,day,file,varname,mini=mini,maxi=maxi,limit=limit,sea=sea,land=land, save_as=save_as,$
	...
	if ts then begin
		datum = '2007-2009' ; use this as first guess
		...
		unit = d.unit
		dum  = strsplit(d.period,'-',/ext)
		date = (dum[0] eq dum[1] ? dum[0] : d.period)+' '
		if keyword_set(limit) and ~keyword_set(coverage) then begin
			bild = (algo2ref(alg) eq 'cci') ? d.mean_cci : d.mean_gac
			ii   = (algo2ref(alg) eq 'cci') ? 8:10
			if keyword_set(reference) then bild2 = (algo2ref(alg) eq 'cci') ? d.mean_gac : d.mean_cci
		endif else begin
			bild = (algo2ref(alg) eq 'cci') ? d.cci_all : d.gac_all
			ii   = (algo2ref(alg) eq 'cci') ? 8:10
			if keyword_set(reference) then bild2 = (algo2ref(alg) eq 'cci') ? d.gac_all  : d.cci_all
		endelse
		fillvalue1 = -999.
		fillvalue2 = -999.
		if keyword_set(reference) then ii = 0
		dumyear  = minmax(where(d.stats[ii,*] ne -999.))/12 + fix((strsplit(d.period,/ext,'-'))[0])
		dummonth = minmax(where(d.stats[ii,*] ne -999.)) mod 12
	...
	if keyword_set(limit) then begin
		if (ts and ~keyword_set(land) and ~keyword_set(sea)) then begin
			...
			if nd_cnt gt 0 then bild[nd_idx] = fillvalue1
			if is_defined(bild2) then begin
				make_geo,grid_res = get_grid_res(bild2),lon,lat
				idx = where(between(lon,limit[1],limit[3]) and between(lat,limit[0],limit[2]),complement=nd_idx,ncomp=nd_cnt)
				if nd_cnt gt 0 then bild2[nd_idx] = fillvalue2
			endif
		endelse
	endif
	if (keyword_set(land) or keyword_set(sea)) and ~ts then begin
		make_geo,grid_res = get_grid_res(bild),lon,lat
		dem = get_dem(lon,lat,grid=get_grid_res(bild))
		if keyword_set(sea) then bild[where(dem ne 0)] = fillvalue1
		if keyword_set(land) then bild[where(dem eq 0)] = fillvalue1
		if is_defined(bild2) then begin
			make_geo,grid_res = get_grid_res(bild2),lon,lat
			dem = get_dem(lon,lat,grid=get_grid_res(bild2))
			if keyword_set(sea)  then bild2[where(dem ne 0)] = fillvalue2
			if keyword_set(land) then bild2[where(dem eq 0)] = fillvalue2
		endif
	endif
	...
	start_save,save_as,thick=thick,snapshot=(float(opl)?'png':'')
		...
		if ~opl then begin
			...
			if is_defined(bild2) then begin
				...
				legend,date+sat_name(algo,sat,year=year,month=month)+hct,thick=thick,spos=(keyword_set(change_side) ? 'bot':'top'),$
			...
			legend,date+sat_name(algo,sat,year=year,month=month)+hct,thick=thick,color=cgcolor(cols), $
...
; 	datum = string(year[0],f='(i4.4)')+string(month[0],f='(i2.2)')
	...
	if keyword_set(timeseries) then begin
		date = '2007-2009' ; use this as first guess
		...
		dum =strsplit(d.period,'-',/ext)
		date = (dum[0] eq dum[1] ? dum[0] : d.period)+' '
		bild = (algo2ref(algo) eq 'cci') ? d.mean_cci : d.mean_gac
		ii   = (algo2ref(algo) eq 'cci') ? 8 : 10
		make_geo,grid_res=get_grid_res(bild),lon,lat
		fillvalue = -999.

		if keyword_set(reference) or keyword_set(old_ts) then ii = 0
		dumyear  = minmax(where(d.stats[ii,*] ne -999.))/12 + fix((strsplit(d.period,/ext,'-'))[0])
		dummonth = minmax(where(d.stats[ii,*] ne -999.)) mod 12
		date     = strjoin(string(dumyear,f=('(i4.4)'))+string(dummonth+1,f=('(i2.2)')),'-')+' '
; date=''
	endif else begin
		bild = get_data(year,month,day,file=file,data=varname,no_data_value=fillvalue,minvalue=minvalue,algo=algo,sat=satellite, $
	...
	medi = zonal_average(bild[where(lat ne fillvalue)],lat[where(lat ne fillvalue)],fillvalue=fillvalue,lat_zon=lat1d,/mean);,/median)
	...
	start_save,save_as,thick=thick,snapshot=(float(opl)?'png':'')
		...
		if ~opl then begin
			...
			charthick = charthick, xcharsize = xcharsize, ycharsize= ycharsize,xmargin=xmargin,ymargin=ymargin
			if chk_idx gt 0 then begin
				if between(count,1,10) then begin
					oplot,lat1d[idx[0:split[0]]],medi[idx[0:split[0]]],thick=thick+2
					for i=1,count do begin
						if i eq count then oplot,lat1d[idx[(split[i-1]+1):*]],medi[idx[(split[i-1]+1):*]],thick=thick+2 else $
						oplot,lat1d[idx[(split[i-1]+1):split[i]]],medi[idx[(split[i-1]+1):split[i]]],thick=thick+2
					endfor
				endif else oplot,lat1d[idx],medi[idx],thick=thick+2
			endif
			legend,date+satn+hct,thick=thick,spos='top',charsize=lcharsize,color =-1,charthick=charthick
		endif else begin
			define_oplots, opl, cols, spos, linestyle, psym, ystretch, error=error
			if chk_idx gt 0 then begin
				if between(count,1,10) then begin
					oplot,lat1d[idx[0:split[0]]],medi[idx[0:split[0]]],thick=thick,col=cgcolor(cols),linestyle=linestyle,psym=psym
					for i=1,count do begin
						if i eq count then oplot,lat1d[idx[(split[i-1]+1):*]],medi[idx[(split[i-1]+1):*]],thick=thick,$
						col=cgcolor(cols),linestyle=linestyle,psym=psym else $
						oplot,lat1d[idx[(split[i-1]+1):split[i]]],medi[idx[(split[i-1]+1):split[i]]],thick=thick,$
						col=cgcolor(cols),linestyle=linestyle,psym=psym
					endfor
				endif else oplot,lat1d[idx],medi[idx],thick=thick,col=cgcolor(cols),linestyle=linestyle,psym=psym
			endif
; 			if chk_idx gt 0 then oplot,lat1d[idx],medi[idx],thick=thick,col=cgcolor(cols),linestyle=linestyle,psym=psym
			legend,date+satn+hct,thick=thick,color=cgcolor(cols), spos=spos,ystretch=ystretch*1.1,charsize=lcharsize,$
...
;-------------------------------------------------------------------------------------------------------------------------
pro plot_simple_timeseries, year,month, varname, sat, algo, cov, reference = reference, mini=mini, maxi=maxi,$
			    ...
			    datum = datum, show_values = show_values
	...
	if keyword_set(structure) then begin
		...
		start_year = keyword_Set(year) 		? year		: 1982 ; 2007
		anz        = keyword_Set(n_months)   	? n_months	: 12   ; 36

		if fix(anz)/12 ne anz/12. then begin
			print, "Anzahl der Monate muss ganzzahlig durch 12 teilbar sein."
			return
		endif

		yy =  indgen((fix(anz)/12)+1)+start_year
		xtickname = strcompress(yy,/rem)

		zeitraum = xtickname[0]+'-'+xtickname[(anz/12)-1]

		d = get_available_time_series( algo, varname, sat, coverage = cov, reference = reference, period = zeitraum, sav_file = sav_file, found = found)
...
;    		corr  = restore_var('/cmsaf/cmsaf-cld1/sstapelb/savs/time_series/compare_cfc_cci_vs_gac_time_series_cci_correction_1982-2014.sav')
;  		corr  = restore_var('/cmsaf/cmsaf-cld1/sstapelb/savs/time_series/compare_cfc_cci_vs_gac_time_series_cci_correction_1982-2014_wo_pinatubu.sav')
		...
		;---------------------------------------------------
 		; CFC Uncertainty, here is the trick, 
		; we devide the cci cfc uncertainty by 5 to decrease it! cool!
; 		dumidx=where(ts_data[9,*] eq -999.,dumidx_cnt)
; 		ts_data[9,*] /= 5.
; 		if dumidx_cnt gt 0 then ts_data[9,dumidx] = -999.
		;---------------------------------------------------
	endif
	opl   = keyword_set(oplots) ? fix(oplots) : 0
	dumyear  = min(where(ts_data[0,*] ne -999.))/12+zeitraum[0]
	dummonth = min(where(ts_data[0,*] ne -999.)) mod 12
	algon = keyword_set(reference) ? sat_name(reference,sat,year=dumyear,month=dummonth,version=version) : sat_name(algo,sat,year=dumyear,month=dummonth,version=version)

	if keyword_set(save_as) then begin
		save_as = !save_dir +'/timeseries/'+file_basename(sav_file,'.sav')+'.eps'
	endif else if win_nr ne -1 then win, win_nr,title=dat

	no_compare = algo2ref(algo) ne 'cci'
	hist_data1 = keyword_set(reference) ? d.CCI_ALL  : -999
	hist_data2 = keyword_set(reference) ? d.GAC_ALL  : -999
	mean_data1 = keyword_set(reference) ? d.MEAN_CCI : -999
	mean_data2 = keyword_set(reference) ? d.MEAN_GAC : -999
	yrange     = minmax(ts_data[0,*],no=-999)
	qu         = 1.
	...
	if anz lt n_elements(ts_data[0,*]) then begin
		...
		anf = ((dum[0]-ori_period[0])*12)
		eef = ((dum[1]-ori_period[0]+1)*12-1)
		if dum[0] gt ori_period[0] then ts_data[*,0:(anf-1)] = -999.
		if ori_period[1] gt dum[1] then ts_data[*,(eef+1):*] = -999.
		anz = [anf,eef+1]
	endif else anz = [0,anz]

	if keyword_set(reference) then begin
		idx = where(ts_data[0,*] ne -999.,idxcnt)
		if idxcnt gt 0 then begin
			str_pholder = strjoin(replicate(' ',max([strlen(sat_name(algo,sat)),strlen(algon)])))
			print,'Glob. Mean    '+sat_name(algo,sat) +': ',string(mean(ts_data[0,idx]),f='(f11.4)')
			print,'Glob. Mean    '+algon              +': ',string(mean(ts_data[1,idx]),f='(f11.4)')
			print,'Glob. BIAS    '+str_pholder        +': ',string(mean(ts_data[6,idx]),f='(f11.4)')
			print,'Glob. RMSE    '+str_pholder        +': ',string(mean(ts_data[7,idx]),f='(f11.4)')
			print,'Glob. BC-RMSE '+str_pholder        +': ',string(mean(sqrt((ts_data[7,idx])^2 - (ts_data[6,idx])^2)),f='(f11.4)')
			print,'minmax BC-RMSE'+str_pholder        +': ',string(minmax(sqrt((ts_data[7,idx])^2 - (ts_data[6,idx])^2)),f='(f11.4)')
			print,'      BIAS    '+str_pholder        +': ',string(mean(ts_data[2,idx]),f='(f11.4)')
			print,'      RMSE    '+str_pholder        +': ',string(mean(ts_data[3,idx]),f='(f11.4)')
			print,'STDDEV(Diff)  '+str_pholder        +': ',string(mean(ts_data[4,idx]),f='(f11.4)')
		endif
	endif
	...
	month='01'
	bin  = 1.
	bin  = (yrange[1] le 0.5 ? 0.001 : (yrange[1] le 1 ? 0.01 : (yrange[1] lt 10 ? 0.1 : 1)))
	lines      = pgrid(yrange)
	min_a      = 0.
	max_a      = 100.
if sat eq 'noaa11' and dat eq 'ctp' then begin
ts_data[8,153] = -999.
ts_data[9,153] = -999.
endif
	if keyword_set(reference) then free,no_compare
	gac_ts_plots,hist_data1,hist_data2,mean_data1,mean_data2,ts_data,bin,strupcase(dat),d.unit,yrange,lines,$
	min_a,max_a,anz,xtickname,qu,algon,log=logarithmic,save_as=save_as,sat,/single_var,thick=thick,error=error,$
	show_values = show_values,no_compare=no_compare,opl=opl,longname=longname,hct=hct,white_bg=white_bg,$
	standard=stregex(varname,'_std',/bool,/fold),zonal_only=zonal_only, datum = datum

	if keyword_set(verbose) then begin
		print,'----'+dat+'------'
		print,'Mean CCI : ', mean(ts_data[0,where(ts_data[0,*] ne -999)])
		if keyword_set(error) then print,'Mean CCI UNC : ', mean(ts_data[5,where(ts_data[5,*] ne -999)])
		print,'Mean '+algon+' : ', mean(ts_data[1,where(ts_data[1,*] ne -999)])
		print,'Mean Bias : ', mean(ts_data[2,where(ts_data[0,*] ne -999)])
		print,'Mean RMSE : ', mean(ts_data[3,where(ts_data[0,*] ne -999)])
		print,'Mean STDD : ', mean(ts_data[4,where(ts_data[0,*] ne -999)])
	endif
end
;-------------------------------------------------------------------------------------------------------------------------
pro boxplot, year, month, day, data=data, satellite = satellite, timeseries = timeseries, limit = limit, coverage = coverage	, $
	...
	case cov of
		...
		'antarctica'	: dem[where(between(lat,-90.0,-65.5),/null)] = 9999.
		'midlat_south'	: dem[where(between(lat,-65.5,-23.5),/null)] = 9999.
		'tropic'	: dem[where(between(lat,-23.5, 23.5),/null)] = 9999.
		'midlat_north'	: dem[where(between(lat, 23.5, 65.5),/null)] = 9999.
		'arctic'	: dem[where(between(lat, 65.5, 90.0),/null)] = 9999.
	...
	if ts then begin
		...
		cov = cov eq 'full' ? '' : cov+'_'
		datum = '1982-2014'
		d = get_available_time_series( algo, dat, sat, reference = reference, coverage = cov, period = datum, found = found)
		if found then begin
			cci = percentile(d.cci_all,[.5,.75,.25,.975,.025],no_data_value=-999.)
			ref = percentile(d.gac_all,[.5,.75,.25,.975,.025],no_data_value=-999.)
			unit = d.unit
			longname = d.longname
			; true period
			dum   = fix(strsplit(d.period,'-',/ext))
			date  = strarr(n_elements(d.STATS[0,*]))
			for i = dum[0],dum[1] do date[(i-dum[0])*12:((i-dum[0]+1)*12)-1] = string(i,f='(i4.4)') + string(indgen(12)+1,f='(i2.2)')
			idx = where(d.STATS[0,*] ne -999.,idxcnt)
			if idxcnt gt 0 then datum = date[min(idx)]+'-'+date[max(idx)]
		...
		name  = [sat_name(algo,sat)+apx1,sat_name(reference,sat)+apx2]
		array = [[temporary(cci)],[temporary(ref)]]
	...
	if keyword_set(file) then begin
		...
		if found then begin
			...
			if found then begin
				cot_array  = dum.cot
				ctp_array  = dum.ctp
				name_arr   = pref+sat_name(algo1,sat)
	...
	if keyword_set(file2) then begin
		...
		if found then begin
			...
			if found then begin
				...
				name_arr   = is_defined(name_arr)  ? [name_arr,pref2+sat_name(algo2,sat)] : pref2+sat_name(algo2,sat)
...
; 	algon_list = ['cci-aqua','cci-terra','cci_noaa12','cci-noaa15','cci-noaa17','cci-metopa','cci-aatsr','cci-aatme','cci-avhrrs','cci-modises','cci-allsat']
	algon_list = [	'cci-noaa7','cci-noaa9','cci-noaa11','cci-noaa14','cci-noaa16','cci-noaa18','cci-noaa19', $
			'gac-noaa7','gac-noaa9','gac-noaa11','gac-noaa14','gac-noaa16','gac-noaa18','gac-noaa19', $
			'gac-noaa12','gac-noaa15','gac-noaa17','gac-metopa','gac-allsat'			, $
			'myd','mod','myd2','mod2','cci_old-noaa15','pmx-noaa7','pmx-noaa12'			, $
			'cci_old-noaa16','cci_old-noaa17','cci_old-noaa18','cci_old-aqua','cci_old-terra']

	years      = string(indgen(33)+1982,f='(i2.2)')
; 	years      = ['2007','2008','2009']
	months     = string(indgen(12)+1,f='(i2.2)')
	plot_l3
	datestr = years[0]+'-'+(reverse(years))[0]
	temp_dir = '/cmsaf/cmsaf-cld5/esa_cci_cloud_data/data/feedback_loops/loop3/' ; cci temporäres verzeichnis von cci
 	for k = 0,n_elements(algon_list) -1 do begin
		dum = strsplit(algon_list[k],'-',/ext)
		ref = dum[0]
		if ref eq 'cci' then dirname=temp_dir
		...
		for i = 0,n_elements(years) -1 do begin
			...
			for j = 0,n_elements(months) -1 do begin
				...
				if ref eq 'gwx' or ref eq 'pmx' then begin
					...
					bla = get_data(yyyy,sat=sat,algo=ref,data='hist2d_cot_ctp',found = found_all)
		...
		if n_elements(final_array_all) ne 0 then begin
			print,'Save -> '+!SAVS_DIR +'cot_ctp_hist2d/cot_ctp_hist2d_'+datestr+'_'+ref+'_'+sat+'.sav'
			save_var,final_array_all,!SAVS_DIR + 'cot_ctp_hist2d/cot_ctp_hist2d_'+datestr+'_'+ref+'_'+sat+'.sav'
			free,final_array_all
		endif
		if n_elements(final_array_liq) ne 0 then begin
			print,'Save -> '+!SAVS_DIR +'cot_ctp_hist2d/cot_ctp_hist2d_liq_'+datestr+'_'+ref+'_'+sat+'.sav'
			save_var,final_array_liq,!SAVS_DIR + 'cot_ctp_hist2d/cot_ctp_hist2d_liq_'+datestr+'_'+ref+'_'+sat+'.sav'
			free,final_array_liq
		endif
		if n_elements(final_array_ice) ne 0 then begin
			print,'Save -> '+!SAVS_DIR +'cot_ctp_hist2d/cot_ctp_hist2d_ice_'+datestr+'_'+ref+'_'+sat+'.sav'
			save_var,final_array_ice,!SAVS_DIR + 'cot_ctp_hist2d/cot_ctp_hist2d_ice_'+datestr+'_'+ref+'_'+sat+'.sav'
...
pro do_1d_hist_time_series
	algon_list = [	'cci-aqua','cci-terra','cci-noaa15','cci-noaa16','cci-noaa17','cci-noaa18','cci-aatme', $
			'cci-aatsr','myd','mod', 'myd2']
	algon_list = [	'cci-aatsr','cci-aatme'];,'cci-aqua','cci-noaa17','cci-noaa18','myd','mod','myd2']
	prod_list  = ['cwp','cot','ctp','ctt','ref']
	years      = ['2008']
	months     = string(findgen(12)+1,f='(i2.2)')
	plot_l3
	datestr = years[0]+'-'+(reverse(years))[0]
	temp_dir = '/cmsaf/cmsaf-cld5/esa_cci_cloud_data/data/feedback_loops/loop3/' ; cci temporäres verzeichnis von cci
	...
	for k = 0,n_elements(algon_list) -1 do begin
		...
		for p = 0,n_elements(prod_list) -1 do begin
			...
			if n_elements(final_array) ne 0 then begin
				print,'Save -> '+!SAVS_DIR + 'hist1d/hist1d_'+dat+'_'+datestr+'_'+ref+'_'+sat+'.sav'
				save_var,final_array,!SAVS_DIR + 'hist1d/hist1d_'+dat+'_'+datestr+'_'+ref+'_'+sat+'.sav'
				free,final_array
			endif
			if n_elements(final_array_ice) ne 0 then begin
				print,'Save -> '+!SAVS_DIR + 'hist1d/hist1d_'+dat+'_ice_'+datestr+'_'+ref+'_'+sat+'.sav'
				save_var,final_array_ice,!SAVS_DIR + 'hist1d/hist1d_'+dat+'_ice_'+datestr+'_'+ref+'_'+sat+'.sav'
				free,final_array_ice
			endif
			if n_elements(final_array_liq) ne 0 then begin
				print,'Save -> '+!SAVS_DIR + 'hist1d/hist1d_'+dat+'_liq_'+datestr+'_'+ref+'_'+sat+'.sav'
				save_var,final_array_liq,!SAVS_DIR + 'hist1d/hist1d_'+dat+'_liq_'+datestr+'_'+ref+'_'+sat+'.sav'
				free,final_array_liq
			endif
		endfor
		free,dirname
...
; 	algon_list = ['cci-aqua','cci-terra','cci_noaa12','cci-noaa15','cci-noaa17','cci-metopa','cci-aatsr','cci-aatme','cci-avhrrs','cci-modises','cci-allsat']
	...
	for k = 0,n_elements(algon_list) -1 do begin
		...
		ref    = dum[0]
		sat    = n_elements(dum) eq 2 ? dum[1] : ''
		data   = ['cfc','ctp','cot','ctt','ref','cwp','iwp','lwp','cph']
		case ref of 
			'cci'		: grid = 0.5
			'cci_old'	: grid = 0.5
			'gac'		: grid = 0.25
		...
		for dd = 0,n_elements(data)-1 do begin
			if data[dd] eq 'ref' and (ref eq 'gac' or ref eq 'gac2') then continue
			...
			for yy=0,nyears-1 do begin
				...
				for mm=0,nmonths-1 do begin
					dum = 	get_data(years[yy],months[mm],data=data[dd],sat=sat,algo=ref,found=found,no_data_value=no_data_value,$
						dirname=dirname,/make_compare,/silent)
					if found then begin
			...
			print,'Saved -> '+!SAVS_DIR + 'hovmoeller/'+data[dd]+'_hovmoeller_'+datestr+'_'+ref+'_'+sat+'.sav'
			save_var,{all:matrix_all,land:matrix_land,sea:matrix_sea} ,!SAVS_DIR + 'hovmoeller/'+data[dd]+'_hovmoeller_'+datestr+'_'+ref+'_'+sat+'.sav'
			free,matrix_all
			free,matrix_land
			free,matrix_sea
		endfor
	...
	!p.multi=[0,7,2]
	win,1,/full
	aa = file_search(!SAVS_DIR +'hovmoeller/cfc_hovmoeller_*')
	ff = aa[[6,7,8,9,3,10,1,12,13,14,15,17,16,0]]
	for i = 0,n_elements(ff)-1 do begin & $
		file   = ff[i] & $
		dum    = strupcase(reverse(strsplit(file_basename(file,'.sav'),/ext,'_'))) & $
		if dum[0] eq 'MOD' then dum = ['TERRA','COLL5'] & $
		if dum[0] eq 'MYD' then dum = ['AQUA' ,'COLL5'] & $
		matrix = transpose(restore_var(file)) & $
		idx = where(matrix le 0,icnt) & $
		if icnt gt 0 then matrix[idx]=0. & $
		xtickname = ['2007','2008','2009','2010'] & $
		ytickname = ['-90','-60','-30','0','30','60','90'] & $
		make_cool_contour,matrix,findgen(36),findgen(180),11,/lines, charsize=2,xmargin=[8,3],ymargin=[5,2], $
;		view2d, matrix,$
		title=dum[1]+' '+dum[0],bar_title='Cloud Fraction',no_data_idx=where(matrix le 0.), $
		ytickname=ytickname,yticks=n_elements(ytickname)-1, ytitle='Latitude', $
		xtickname=xtickname,xticks=n_elements(xtickname)-1,min=10.,max=100,color=0,col_tab=4 & $;col_tab=105,color=(i/7 eq i/7. ? 1:0 ) & $
		for j = 1,5 do oplot,!x.crange,[j*30,j*30],linestyle=1 & $
		for j = 1,2 do oplot,[(j*12),(j*12)],!y.crange,linestyle=2 & $
	endfor
end
; ----------------------------------------------------------------------------------------------------------------------------------------------
; ----------------------------------------------------------------------------------------------------------------------------------------------