...
PRO DF_COLORBAR, BOTTOM=bottom, CHARSIZE=charsize, COLOR=color, DIVISIONS=divisions, $
   ...
   logarithmic = logarithmic,no_noerase = no_noerase,oob_factor=oob_factor,oob_low=oob_low,oob_high=oob_high
...
thisRelease = Float(!Version.Release)
	...
	;g_eq and l_eq eingefuegt 25.7. 2012 stapel
	if total(stregex(tag_names(extra),'g_eq',/fold,/bool)) then g_eq = extra.g_eq
	if total(stregex(tag_names(extra),'l_eq',/fold,/bool)) then l_eq = extra.l_eq
	if total(stregex(tag_names(extra),'bar_tickname',/fold,/bool)) then b_tn = extra.bar_tickname

	oob_factor =  (n_elements(oob_factor) ne 0) ? oob_factor : 1.
	oob_low    =   keyword_set(l_eq) ? '2'   : 0
	oob_high   =   keyword_set(g_eq) ? '254' : 0
...
maxrange = (keyword_set(logarithmic) ? (10.^maxrange) : maxrange)
minrange = (keyword_set(logarithmic) ? (10.^minrange) : minrange)

IF (minrange eq maxrange) then maxrange = minrange +1
...
ENDELSE
     ...
     IF (N_Elements(oob_high) NE 0) || (N_Elements(oob_low) NE 0) THEN BEGIN
         IF Keyword_Set(vertical) THEN BEGIN
            ...
            IF keyword_set(oob_high) THEN position[3] = position[3] - length
            IF keyword_set(oob_low)  THEN position[1] = position[1] + length
	ENDIF ELSE BEGIN
            length = (position[3]-position[1]) * oob_factor
            IF keyword_set(oob_high) THEN position[2] = position[2] - length
            IF keyword_set(oob_low)  THEN position[0] = position[0] + length
         ENDELSE
     ENDIF
...
;   bar = BYTSCL(bar, TOP=(ncolors-1 < (255-bottom))) + bottom
  bar = keyword_set(l_eq) > BYTSCL(bar, TOP=((ncolors-1-keyword_set(g_eq)) < (255-bottom))) + bottom
...
ENDIF ELSE BEGIN
   ...
   if keyword_set(no_noerase) then plot,[0,0],/nodata,position=position,xstyle=4,ystyle=4
...
ENDELSE
   ...
   ; Annotate the color bar.
	...
	dum = vector(minrange,maxrange,divisions+1,logarithmic=logarithmic)
	ytickname = strcompress(string(dum,f=format),/rem)
	xtickname = strcompress(string(dum,f=format),/rem)
	if keyword_set(b_tn) then begin
		divisions *= 2
		xtickname = strarr(divisions+1) + ' '
		xtickname[1:*:2] = b_tn
		ytickname = xtickname
	endif

	if keyword_set(l_eq) then begin
		dummi = min(dum, min_idx)
		xtickname[min_idx] = textoidl('\leq') + ' ' + xtickname[min_idx]
		ytickname[min_idx] = textoidl('\leq') + ' ' + ytickname[min_idx]
	endif
	if keyword_set(g_eq) then begin
		dumma = max(dum, max_idx)
		xtickname[max_idx] = textoidl('\geq') + ' ' + xtickname[max_idx]
		ytickname[max_idx] = textoidl('\geq') + ' ' + ytickname[max_idx]
	endif
...
IF KEYWORD_SET(vertical) THEN BEGIN
   ...
   IF KEYWORD_SET(right) THEN BEGIN
      ...
      PLOT, [minrange,maxrange], [minrange,maxrange], /NODATA, XTICKS=1, $
         YTICKS=divisions, XSTYLE=1, YSTYLE=9, ylog = logarithmic, $
	...
	; ytickformat mit ytickname ausgetauscht
      AXIS, YAXIS=1, YRANGE=[minrange, maxrange],ytickname=ytickname , YTICKS=divisions, $
         YTICKLEN=ticklen, YSTYLE=1, COLOR=color, CHARSIZE=charsize, $
         FONT=font, YTITLE=title, _EXTRA=extra, YMINOR=minor, ylog = logarithmic

   ENDIF ELSE BEGIN

; ytickformat mit ytickname ausgetauscht
      PLOT, [minrange,maxrange], [minrange,maxrange], /NODATA, XTICKS=1, $
         YTICKS=divisions, XSTYLE=1, YSTYLE=9, YMINOR=minor, ylog = logarithmic, $
         POSITION=position, COLOR=color, CHARSIZE=charsize, /NOERASE, $
         ytickname=ytickname, XTICKFORMAT='(A1)', YTICKLEN=ticklen , $
      ...
      AXIS, YAXIS=1, YRANGE=[minrange, maxrange], YTICKFORMAT='(A1)', YTICKS=divisions, $
         YTICKLEN=ticklen, YSTYLE=1, COLOR=color, CHARSIZE=charsize, $
         FONT=font, _EXTRA=extra, YMINOR=minor, ylog = logarithmic
...
ENDIF ELSE BEGIN
   ...
   IF KEYWORD_SET(top) THEN BEGIN
      ...
      PLOT, [minrange,maxrange], [minrange,maxrange], /NODATA, XTICKS=divisions, $
         YTICKS=1, XSTYLE=9, YSTYLE=1, xlog = logarithmic, $
         POSITION=position, COLOR=color, CHARSIZE=charsize, /NOERASE, $
         YTICKFORMAT='(A1)', XTICKFORMAT='(A1)', XTICKLEN=ticklen, $
         XRANGE=[minrange, maxrange], FONT=font, _EXTRA=extra, XMINOR=minor
;  xtickformat mit xtickname ausgetauscht
      AXIS, XTICKS=divisions, XSTYLE=1, COLOR=color, CHARSIZE=charsize, xlog = logarithmic, $
         xtickname=xtickname, XTICKLEN=ticklen, XRANGE=[minrange, maxrange], XAXIS=1, $
         FONT=font, XTITLE=title, _EXTRA=extra, XCHARSIZE=charsize, XMINOR=minor, xlog = logarithmic

   ENDIF ELSE BEGIN

   ;  xtickformat mit xtickname ausgetauscht
	PLOT, [minrange,maxrange], [minrange,maxrange], /NODATA, XTICKS=divisions, $
         YTICKS=1, XSTYLE=1, YSTYLE=1, TITLE=title, POSITION=position, COLOR=color, $
         CHARSIZE=charsize, /NOERASE, YTICKFORMAT='(A1)', XTICKLEN=ticklen, $
         XRANGE=[minrange, maxrange], FONT=font, XMinor=minor, _EXTRA=extra, $
	xtickname = xtickname, xlog = logarithmic
...
ENDELSE
   ...
   ; Restore the previous plot and map system variables.
    ...
    IF keyword_set(oob_high) || keyword_set(oob_low) THEN BEGIN
         p = position
         IF Keyword_Set(vertical) THEN BEGIN
