...
pro isccp_read_d2,sqmap,filename,variable,hour,found=found

;A sample IDL program written by the:
;     BADC
...
;     OX11 0QX                     
;     England, UK.                 
;designed to read in an HDF isccp file, in EQ format, remove
;a selected variable, convert them to real values, and then 
;transform them onto a 2.5x2.5 square grid
;09/07/2002 -SMD
;#################################################################
;Select some data to look at
;#################################################################
;filename='ISCCP.D2.0.GLOBAL.1998.02.99.9999.GPC.hdf'   ;choose a file to open
;variable=20    ;select a varaiable. For d1 dataset these are numbered 1-130
;hour=5         ;These can be any of:
;                                  1 = 00:00 GMT
;                                  2 = 03:00 GMT
;                                  3 = 06:00 GMT
;                                  4 = 09:00 GMT
...
;#################################################################
maxbox=6596; - number of boxes for globe
rundef= -1000.0 ; - undefinded data indicatior for reals
maxlat=72 ; - number of latitudes
maxlon=144 ; - number of longitudes
iundef=255 ; - undefined data indicator for integers
PATHW=6.292
;
;We also need to define some vectors that tell us infomation about the grid
;and lookup tables for converting to real numbers. 
loadtab,tmptab,tmpvar,pretab,rfltab,tautab,prwtab,ozntab,ncells,icells
;############################################################
;
;############################################################
;   print,'processing file:',filename
 ;First open the file
 if ~is_hdf(filename) then begin
	found = 0
	return
 endif else found = 1
 id=hdf_sd_start(filename)
;  if id gt 0 then print,'file opened successfully'
 ;open a selected variable
 id_8=HDF_SD_SELECT(id,hour-1)
 start=[0,0,variable-1,0]
 count=[0,0,0,maxbox]
 ;Get the slab of data for the variable we have chosen
 HDF_SD_GETDATA,id_8,data,start=start,count=count
 HDF_SD_END,id
 xy=size(data)
 rvar=fltarr(xy(2))
;Convert the data slab to real values
;#############################################################
;First we are going to work on reading in the ancillary data
lonlim=intarr(2,maxbox)
string=' '
get_lun,unit
openr,unit,'/cmsaf/nfshome/sstapelb/idl/isccp/d2ancil.data.txt'
readf,unit,string
readf,unit,string
i=0
dummy=' '
while (not eof(unit)) do begin
;   read the transformation indicies
    readf,unit,f1,f2,f3,f4,f5,format='(5I4)' 
    lonlim(0,i)=f4
    lonlim(1,i)=f5
    i=i+1
  end
...
free_lun,unit
;#############################################################
;different coversions required for different fields.
;#############################################################
 if variable le 7 then begin
  for box=0,maxbox-1 do begin
       if data(box) eq iundef then begin
          rvar(box)=rundef
       endif else begin
          rvar(box)=float(data(box))
       endelse
  endfor
 endif
;
 if variable ge 8 and variable le 19 then begin
  for box=0,maxbox-1 do begin
       if data(box) eq iundef then begin
          rvar(box)=rundef
       endif else begin
          rvar(box)=data(box)*0.5
       endelse
  endfor
 endif
;
 if variable ge 20 and variable le 22 then begin
  for box=0,maxbox-1 do begin
     rvar(box)=pretab(data(box))
  endfor
 flag=1
 endif
;
 if variable ge 23 and variable le 24 then begin
  for box=0,maxbox-1 do begin
     rvar(box)=tmptab(data(box))
  endfor
 endif
 if variable eq 25 then begin
  for box=0,maxbox-1 do begin
     rvar(box)=tmpvar(data(box))
  endfor
 endif
;
 if variable ge 26 and variable le 31 then begin
  for box=0,maxbox-1 do begin     
     rvar(box)=tautab(data(box))
     if variable ge 29 and variable le 31 and rvar(box) ge 0 then $
            rvar(box) = rvar(box) * PATHW
  endfor
 endif
;
 if variable eq 32 or variable eq 35 or variable eq 38 then begin
  for box=0,maxbox-1 do begin     ;stapel included
     if data(box) eq iundef then begin
          rvar(box)=rundef
     endif else begin
          rvar(box)=data(box)*0.5
     endelse   
  endfor    ;stapel included
 endif 
 if variable eq 33 or variable eq 36 or variable eq 39 then $
              rvar(box)=pretab(data(box))
 if variable eq 34 or variable eq 37 or variable eq 40 then $
              rvar(box)=tmptab(data(box))
;
 if variable ge 41 and variable le 115 then begin
  test=variable-round(variable/10)*10
  print,test
  if test eq 1 or test eq 6 then begin
   for box=0,maxbox-1 do begin
      if data(box) eq iundef then begin
           rvar(box)=rundef
      endif else begin
  ...
  if test eq 2 or test eq 7 then begin
      for box=0,maxbox-1 do begin
        rvar(box)=pretab(data(box))
      endfor
  endif
  if test eq 3 or test eq 8 then begin
      for box=0,maxbox-1 do begin
        rvar(box)=tmptab(data(box))
      endfor
  endif
  if test eq 4 or test eq 9 then begin
      print,'boo'
      for box=0,maxbox-1 do begin
        rvar(box)=tautab(data(box))
      endfor
  endif
  if test eq 5 or test eq 0 then begin
      for box=0,maxbox-1 do begin
        rvar(box)=tautab(data(box))
        if rvar(box) ge 0.0 then rvar(box)=rvar(box)*PATHW
      endfor
  endif
endif
;############################################################# 
;Now transform to a square grid
;#############################################################
 ibox=-1
 sqmap=fltarr(maxlon,maxlat)
 for lat=0,maxlat-1 do begin ;loop over 72 latitude bands
  for lon=0,icells(lat)-1 do begin ;loop over the number of cells in a lat band
   ibox=ibox+1                     ;counter for total cells
   lonsq1=lonlim(0,ibox) ;sq1 ; these two define the range of true lats for cell ibox
   lonsq2=lonlim(1,ibox) ;sq2 ; to be applied to
     for ilon=lonsq1-1,lonsq2-1 do begin
;       lonsq=ilon+maxlon/2   ;corrects data so that cell 1 is zero degrees longitude
;       if lonsq gt maxlon-1 then lonsq = lonsq-maxlon
; BUT we're going to do it so that 0 degrees is out first longitude (UM compatible)
         lonsq=ilon
         sqmap(lonsq,lat)=rvar(ibox)
     endfor
  endfor
 endfor
;######################################################################
;And finally the data can be plotted but you can do whatever you like:
;This plot will only look any good for cloud amounts.
; x=indgen(maxlon)*2.5
; y=indgen(maxlat)*2.5-90.0
;print,x
; loadct,13
;contour,sqmap,x,y,max_value=100,min_value=0,nlevels=100,yrange=[-90,90],ystyle=1,/fill
end
;#######################################################################
pro loadtab,tmptab,tmpvar,pretab,rfltab,tautab,prwtab,ozntab,ncells,icells
      tmptab=[$                                        
