;-------------------------------------------------------------------------------------------------------------------------
function get_grid_res, data, found = found, cci_l3u_eu = cci_l3u_eu, claas = claas
	cci_l3u_eu=0.
	claas=0.
	if n_elements(data) gt 0 then begin
		...
		if size(dumdata,/n_dim) eq 2 then begin
			...
			if dum[0] eq dum[1] then begin ; regular grid?
				if total(dum[0] eq [0.02,0.05,0.10,0.25,0.50,0.75,1.00,2.00,2.50,5.00]) then return,dum[0] ; CCI vali standards
