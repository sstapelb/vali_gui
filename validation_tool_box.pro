...
function get_product_name, data, algo=algo, upper_case = upper_case, lower_case = lower_case, level = level,path=path ,h_types=h_types
	...
	if total(alg eq ['gac2','clara2']) then begin
		if keyword_set(path) then begin
			...
			if dat eq 'cot_ice' and lev eq 'l3c' then dat = 'iwp'
			if dat eq 'cot_liq' and lev eq 'l3c' then dat = 'lwp'
			if dat eq 'ref_ice' and lev eq 'l3c' then dat = 'iwp'
			if dat eq 'ref_liq' and lev eq 'l3c' then dat = 'lwp'
			if dat eq 'cer_ice' and lev eq 'l3c' then dat = 'iwp'
			if dat eq 'cer_liq' and lev eq 'l3c' then dat = 'lwp'
	...
	if alg eq 'esacci' or alg eq 'cci' then begin
		case dat of
			...
			;------------------------------------------
			'cfc'			: dat = 'cc_total'	;remove this for version 2.0
			'cfc_day'		: dat = 'cc_total_day'	;remove this for version 2.0
			'cfc_night'		: dat = 'cc_total_night';remove this for version 2.0
			'cfc_twl'		: dat = 'cc_total_twl'	;remove this for version 2.0
			'a_cod'			: dat = 'cot_log'
			'cwp_ice'		: dat = 'iwp'
			'cwp_liq'		: dat = 'lwp'
			'cot_ctp_hist2d'	: dat = 'hist2d_cot_ctp'
	...
	if n_elements(data) gt 0 then begin
		...
		if size(dumdata,/n_dim) eq 2 then begin
			...
			if dum[0] eq dum[1] then begin ; regular grid?
				if total(dum[0] eq [2.5,0.25,0.5,0.1,1.,0.05]) then return,dum[0] ; CCI vali standards
			endif
		endif
	endif
	found=0.
...
pro make_geo, file = file, lon, lat, grid_res = grid_res, verbose = verbose, dimension = dimension, found = found, msg = msg, $
	...
	if keyword_set(msg) or claas then begin
...
; 		if between(fix(orb),0,found-1) then filename = file_search(filename[fix(orb)],count = found)
	...
	if ~found and ~sil then begin
		...
		ok = dialog_message('get_filename: (Database: '+dir+'): No '+alg+' level '+strcompress(lev)+' data files found! Satellite: '+sat+' '+inst+', Date: '+yyyy+'/'+mm+'/'+dd)
...
; old=1
	...
	if keyword_set(reference) then begin
		...
		ref   = algo2ref(algo,sat=sat)
		if ~keyword_set(hovmoeller) then if ref eq 'cci' then ref = 'gac';'cci_old'
		cli   = 'cci' ;  change this to your climatology, e.g 'cci_old'
	endelse
	...
	if keyword_set(hovmoeller) then begin
		dumsat = sat
		if total(strmid(ref,0,3) eq ['myd','mod']) then dumsat = ''
		if ref eq 'pmx' then dumsat = (noaa_ampm(sat) eq 'pm' ? 'noaa7':'noaa12')
		sav_file = !SAVS_DIR + 'hovmoeller/'+old+''+dat+'_hovmoeller_'+per+'_'+ref+'_'+dumsat+'.sav'
		sfile    = file_search( sav_file ,count = found)
		if found eq 0 and keyword_set(period) then begin
			sav_file = !SAVS_DIR + 'hovmoeller/'+old+''+dat+'_hovmoeller_????-????_'+ref+'_'+dumsat+'.sav'
			sfile    = file_search( sav_file ,count = found)
		endif
	endif else begin
		sav_file = !SAVS_DIR + 'time_series/'+old+'compare_'+dat+'_'+cli+'_vs_'+ref+'_time_series_'+sat+(cov eq '' ? '':'_')+cov+'_'+per+'.sav'
		sfile    = file_search( sav_file ,count = found)
		if found eq 0 then begin
			if keyword_set(period) then begin
				sav_file = !SAVS_DIR + 'time_series/'+old+'compare_'+dat+'_'+cli+'_vs_'+ref+'_time_series_'+sat+(cov eq '' ? '':'_')+cov+'_????-????.sav'
				sfile    = file_search( sav_file ,count = found)
			endif
			if found eq 0 and total(ref eq ['myd','mod','myd2','mod2']) then begin
				...
				sav_file = !SAVS_DIR + 'time_series/'+old+'compare_'+dat+'_'+cli+'_vs_'+ref+'_time_series_noaa??'+(cov eq '' ? '':'_')+cov+'_????-????.sav'
				sfile    = file_search( sav_file ,count = found)
			endif
		endif
	endelse
	...
	if found then begin
		if keyword_set(hovmoeller) then begin
			...
			corr  = correlate(struc.cci_all,struc.gac_all)
			stdd_cci = stddev(struc.cci_all)
			stdd_gac = stddev(struc.gac_all)
			struc = create_struct(struc,{period:datum,longname:longname,unit:unit,coverage:cov,correlation:corr,cci_stdd:stdd_cci,gac_stdd:stdd_gac})
		endelse
...
function get_data, year, month, day, orbit=orbit,data=data,satellite=satellite	, $
	...
	if not keyword_set(data) then begin
		print, 'You need to specify the parameter [data=] that you want to extract from the file!'
		found = 0.
		return,-1
	endif
...
; 		usgs_file = '/cmsaf/cmsaf-cld5/esa_cci_cloud_data/usgs_type_dem/Aux_file_CM_SAF_AVHRR_GAC_0.25deg.nc'
		...
		; 1) iwp
		filename = get_filename(year,month,day,data=dat+'_ice', satellite=sat, level=lev,algo=alg,found=found,instrument=instrument,silent=silent,dirname=dirname)
		read_data, filename[0], dat+'_ice', ice, no_data_valuei, minvalue, maxvalue, longname, unit, verbose = verbose, found = found
		if not found then return,-1
		; 2) lwp
		...
		read_data, liq_file, dat+'_liq', liq, no_data_value, minvalue, maxvalue, longname, unit, verbose = verbose, found = found
		if not found then return,-1
		; 3) cph
		cph_file = get_filename(year,month,day,data='cph', satellite=sat, level=lev,algo=alg,found=found,instrument=instrument,silent=silent,dirname=dirname)
		dumdat = get_product_name('cph_day',algo=alg,level=lev)
...
pro set_algolist, algo_list, sat = sat, data = data, default = default, exclude = exclude
	...
	if keyword_set(default) and keyword_set(sat) and keyword_set(data) then begin
		...
		if ampm eq 'am' then begin
			algo_list = co5 ? ['mod2','cci','cci_old'] : ['pmx','gac','gac2','cci','cci_old','mod2']
		endif else begin
			algo_list = co5 ? ['myd2','cci','cci_old'] : ['pmx','gac','gac2','cci','cci_old','myd2']
		endelse
	endif else algo_list = ['cci','cci_old','pmx','myd','myd2','mod','mod2','gac','gac2','gwx','cla','isp','era']
			...
			if found then begin
				...
				if keyword_set(zonal_mean) or keyword_set(percentile) then begin
					if keyword_set(zonal_mean) then begin
						medi = zonal_average(dum,lat,fillvalue=ndv,mean=mean,median=median,/nan)
